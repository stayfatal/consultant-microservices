...
func New(cfg *config.ServicesConfig, logger *log.Logger) (interfaces.Service, error) {
	clientConn, err := grpc.NewClient(fmt.Sprintf("%s:%s", cfg.Matchmaking.Host, cfg.Matchmaking.Port), grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return nil, err
	}
	...
	go func() {
		for {
			...
			if err != nil {
				...
				s.deleteUserWithConn(user.user.Id)
...
func (s *service) deleteUserWithConn(id int) {
	s.mu.Lock()
	delete(s.users, id)
	s.mu.Unlock()
}
